...
trap 'rm -f "$password_file" "$bind_password_file"' EXIT
chmod 0600 "$password_file" "$bind_password_file"
...
printf %s "$LDAP_BIND_PASSWORD" >"$bind_password_file"
...
ldapsearch -LLL -o ldif-wrap=no -x -H "ldap://${BANK_LDAP_IP}:${LDAP_PORT}" \
  -D "$LDAP_BIND_DN" -y "$bind_password_file" \
